...
class play(commands.Cog):
    ...
    async def on_ready(self):
        LogExitoso("[Slash Command] play cargado.").print()

    @app_commands.command(
        name="play", description="Reproduce una canción o agrega a la cola."
    )
    @app_commands.describe(url="URL de la canción a reproducir | agregar a la cola.")
    async def play(self, interaction: discord.Interaction, url: str = None):
        await interaction.response.defer()
        print(
            f"{Fore.BLUE}[info] Comando play ejecutado en '{interaction.guild.name}' por '{interaction.user.name}'."
        )
        ...
        if (
            ...
            and await player.joinVoiceChannel(interaction.user.voice.channel)
            == "connected"
        ):
            if not url:
                await player.joinVoiceChannel(interaction.user.voice.channel)
                if player.VoiceClient is not None and player.queue:
                    await player.play(interaction)
                    await interaction.followup.send(
                        embed=discord.Embed(
                            title="Reproducción reanudada.",
                            description="Agrega mas canciones a la cola proprocionando una URL.",
                ...
                await interaction.followup.send(
                    embed=discord.Embed(
                        title="Me he unido al canal de voz.",
                        description="Necesitas proporcionar una URL para reproducir.",
                        color=Colours.default(),
                    )
                )
                return
