...
class clear(commands.Cog):
    ...
    async def on_ready(self):
        LogExitoso("[Slash Command] clear cargado.").print()
    ...
    async def clear(self, interaction: discord.Interaction):
        ...
        if player:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    title="Se ha limpiado la cola.", color=Color(0x24005A)
                )
            )
            return

        await interaction.response.send_message(
            embed=discord.Embed(title="No hay canciones en la cola.", color=Color.red())
        )
