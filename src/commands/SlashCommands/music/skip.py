...
class skip(commands.Cog):
    ...
    async def on_ready(self):
        LogExitoso(f"[Slash Command] {self.__class__.__name__} cargado.").print()

    @app_commands.command(
        name="skip",
        description="Salta una o varias canciones de la cola de reproducción.",
    )
        ...
        if player:
            if len(player.queue) != 0:
                if posicion > 0 and posicion <= len(player.queue):
                    ...
                    await interaction.response.send_message(
                        embed=discord.Embed(
                            title=f"Se han saltado {posicion} canciones.",
                    ...
                    await interaction.response.send_message(
                        embed=discord.Embed(
                            title=f"Se salto la cancion", color=discord.Color.green()
                        )
                    )
                    await player.stop()
                    await player.play(interaction)
                    ...
                    await interaction.response.send_message(
                        embed=discord.Embed(
                            title="La posición ingresada no es válida.",
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    title="No se ha podido saltar la canción.",
                    color=discord.Color.red(),
                )
            )
            return
