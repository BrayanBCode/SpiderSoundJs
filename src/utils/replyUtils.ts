import { DeferReplyOptions, DeleteReplyOptions, EditReplyOptions, SendReplyOptions, SendUpdateOptions } from "@/types/interface/IReplyUtils.js";
import {
    CommandInteraction, MessageComponentInteraction, ModalSubmitInteraction, ButtonBuilder, ButtonStyle, ActionRowBuilder, Message, ButtonInteraction, AnySelectMenuInteraction, EmbedBuilder
} from "discord.js";
...
export async function sendReply(interaction: CommandInteraction | MessageComponentInteraction | ModalSubmitInteraction, options: SendReplyOptions): Promise<Message | undefined> {
    try {
        ...
        if (options.components) {
            options.components = options.components.map(component => component instanceof ActionRowBuilder ? component.toJSON() as any : component);
        }
