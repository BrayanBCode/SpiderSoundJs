...
import { chunkArray, createEmptyEmbed, formatMS_HHMMSS } from "@/utils/tools";
import { ChatInputCommandInteraction, ButtonInteraction, TextChannel, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, Message, type InteractionReplyOptions } from "discord.js";
            ...
            (tracks, index) => {
                return createEmptyEmbed()
                    ...
                    .addFields(...tracks.map((track, subIndex) => {
                        ...
                        return {
                            name: `🎵${globalIndex}. ${track.title}`,
                            value: `duración: ${formatMS_HHMMSS(track.duration)}`
                        }
                    }))
            })
        )
