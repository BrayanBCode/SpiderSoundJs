...
class PaginationView(discord.ui.View):
    ...
    async def get_embed(self):
        ...
        embed = Embed(title=f'Reproduciendo actual: {self.playing_song["title"]} de {self.playing_song["author"]}\n Duracion: {self.playing_song["duration"]}', color=0x120062)
    ...
    @discord.ui.button(label='Anterior', style=discord.ui.ButtonStyle.primary)
    async def previous_page(self, button: discord.ui.Button, interaction: discord.ui.Interaction):
        if self.current_page > 0:
            self.current_page -= 1
            embed = await self.get_embed()
            await interaction.response.edit_message(embed=embed)

    @discord.ui.button(label='Siguiente', style=discord.ui.ButtonStyle.primary)
    async def next_page(self, button: discord.ui.Button, interaction: discord.ui.Interaction):
